...
var ContractDiscovery = class {
  ...
  async getActiveNodes() {
    try {
      const nodeIds = await this.contract.getActiveNodes();
      const nodes = [];
...
Timestamp: ${timestamp}
Nonce: ${nonce}`;
        const signature = await signer.signMessage(message2);
        authorization = {
          sender,
          signature,
          timestamp,
  ...
  async isContentRegistered(cid) {
    ...
    try {
      const provider = new ethers2.JsonRpcProvider(this.config.rpcUrl);
      const contract = new ethers2.Contract(
        this.config.contentRegistryAddress,
        CONTENT_REGISTRY_ABI,
        provider
  ...
  async handlePeerAnnouncement(announcement) {
   ...
   */
  async sendSignalingMessage(targetPeerId, signal) {
    if (!this.node) return;
    const pubsub = this.node.services.pubsub;
    if (!pubsub) return;
  ...
  findPeerForContentType(contentType) {
    for (const peer of this.knownPeers.values()) {
      if (!peer.connected) continue;
      if (!peer.isRegistered) continue;
...
function ByteCaveProvider({
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const peers2 = await globalClient.getPeers();
  ...
  useEffect(() => {
    ...
    const initializeClient = async () => {
      ...
      const handlePeerUpdate = async () => {
        if (!client) return;
        try {
          const peers2 = await client.getPeers();
          setPeers(peers2);
        } catch (err) {
        }
      };
  ...
  const getNodeHealth = async (peerId) => {
    if (!globalClient) {
      return null;
    }
